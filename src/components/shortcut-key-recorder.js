...
export function ShortcutKeyRecorder({ shortcutKey: initialShortcutKey, onChange, className }) {
    const [inRecording, setInRecording] = useState(false);
    const [draftShortcut, setDraftShortcut] = useState("");
    const [optimisticShortcut, setOptimisticShortcut] = useState(null);
    const inputRef = useRef(null);
    const isRecordingRef = useRef(false);
    const endRecording = useCallback((nextShortcut) => {
    ...
    const commitShortcut = useCallback((nextShortcut) => {
    ...
    const shortcutKey = optimisticShortcut !== null && optimisticShortcut !== initialShortcutKey
        ? optimisticShortcut
        : initialShortcutKey;
    return (_jsx(Input, { ref: inputRef, className: className, onFocus: startRecord, onBlur: handleBlur, value: formatPageTranslationShortcut(inRecording ? draftShortcut : shortcutKey), placeholder: i18n.t("shortcutKeySelector.placeholder"), readOnly: true }));
}
