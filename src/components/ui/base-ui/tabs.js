...
function Tabs({ className, orientation = "horizontal", onValueChange, ...props }) {
    const handleValueChange = React.useCallback((value, _eventDetails) => {
        onValueChange?.(value);
    }, [onValueChange]);
    return (_jsx(TabsPrimitive.Root, { "data-slot": "tabs", "data-orientation": orientation, className: cn("gap-2 group/tabs flex data-[orientation=horizontal]:flex-col", className), onValueChange: handleValueChange, ...props }));
}
