...
vi.mock("@/components/ui/base-ui/avatar", () => ({
    Avatar: ({ children, className, size = "default", }) => (_jsx("span", { "data-slot": "avatar", "data-size": size, className: className, children: children })),
    AvatarImage: (props) => props.src ? _jsx("img", { "data-slot": "avatar-image", ...props }) : null,
    AvatarFallback: ({ children }) => (_jsx("span", { "data-slot": "avatar-fallback", children: children })),
}));
describe("user account", () => {
    ...
    it("treats a session payload without user as signed out", async () => {
        ...
        const { UserAccount } = await import("../user-account");
        expect(() => render(_jsx(UserAccount, {}))).not.toThrow();
        const image = screen.getByRole("img", { name: "Guest" });
        expect(image).toHaveAttribute("src", guest);
        expect(screen.getByText("Guest")).toBeInTheDocument();
        expect(screen.getByRole("button", { name: "Log in" })).toBeInTheDocument();
    ...
    it("keeps the loading state without showing login", async () => {
        ...
        const { UserAccount } = await import("../user-account");
        const view = render(_jsx(UserAccount, {}));
