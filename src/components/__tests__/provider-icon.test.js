...
const { resolveContentScriptAssetBlobMock, shouldProxyAssetUrlMock, createImageBitmapMock, clearRectMock, drawImageMock, setTransformMock, } = vi.hoisted(() => ({
...
describe("provider icon", () => {
    ...
    it("renders a normal image when the logo does not need proxying", async () => {
        ...
        render(_jsx(ProviderIcon, { logo: "https://cdn.example.com/logo.webp", name: "OpenAI", size: "md" }));
    ...
    it("normalizes local extension asset paths before rendering", async () => {
        shouldProxyAssetUrlMock.mockReturnValue(false);
        const { default: ProviderIcon } = await import("../provider-icon");
        render(_jsx(ProviderIcon, { logo: "/assets/providers/deeplx-light.svg", name: "DeepLX", size: "md" }));
    ...
    it("renders proxied logos through a canvas instead of an img tag", async () => {
        ...
        const { default: ProviderIcon } = await import("../provider-icon");
        const view = render(_jsx(ProviderIcon, { logo: "https://cdn.example.com/logo.webp", name: "OpenAI", size: "md" }));
        await waitFor(() => {
