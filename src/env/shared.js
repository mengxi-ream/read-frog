...
function parseCommaSeparatedEntries(value, ctx, entrySchema) {
    const parsedEntries = [];
...
export function isLocalPackagesEnabled(rawEnv) {
    return rawBooleanSchema.parse(rawEnv.WXT_USE_LOCAL_PACKAGES) ?? false;
}
export function resolveExtensionEnv(rawEnv) {
...
export function createExtensionClientEnvSchema(isProd, skipRequiredProductionEnv = false) {
    const requiresProductionEnv = isProd && !skipRequiredProductionEnv;
    return {
        WXT_API_URL: strictUrlSchema,
        WXT_WEBSITE_URL: strictUrlSchema,
        WXT_OFFICIAL_SITE_ORIGINS: z.string().transform((value, ctx) => parseCommaSeparatedEntries(value, ctx, strictOriginSchema)),
        WXT_AUTH_COOKIE_DOMAINS: z.string().transform((value, ctx) => parseCommaSeparatedEntries(value, ctx, strictCookieDomainSchema)),
