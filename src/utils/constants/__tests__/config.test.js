...
describe("dEFAULT_CONFIG", () => {
    ...
    it("initializes when crypto.randomUUID is unavailable but crypto.getRandomValues exists", async () => {
        const getRandomValues = vi.fn((array) => originalCrypto.getRandomValues(array));
        Object.defineProperty(globalThis, "crypto", {
            configurable: true,
            value: {
                getRandomValues,
    ...
    it("seeds DeepSeek without the removed legacy provider in the default providers config", async () => {
        ...
        expect(DEFAULT_CONFIG.providersConfig.find(provider => provider.id === "deepseek-default")).toEqual(expect.objectContaining({
            model: {
