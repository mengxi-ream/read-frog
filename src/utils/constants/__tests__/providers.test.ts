...
describe("provider constants", () => {
  ...
  it("defines provider-specific Base URL placeholders", () => {
    ...
    expect(PROVIDER_BASE_URL_PLACEHOLDERS.azure).toBe(
  ...
  it("places Atlas Cloud second in OpenAI-compatible providers", () => {
    expect(CUSTOM_LLM_PROVIDER_TYPES.slice(0, 4)).toEqual([
      "openai-compatible",
      "atlascloud",
      "openrouter",
      "minimax",
    ])
    expect(API_PROVIDER_TYPES.slice(0, 4)).toEqual([
      "openai-compatible",
      "atlascloud",
      "openrouter",
      "minimax",
  ...
  it("places Azure immediately before Bedrock in provider pickers", () => {
    expect(NON_CUSTOM_LLM_PROVIDER_TYPES.indexOf("azure")).toBe(
      NON_CUSTOM_LLM_PROVIDER_TYPES.indexOf("bedrock") - 1,
    )
