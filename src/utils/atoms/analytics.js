...
export const baseAnalyticsEnabledAtom = atom(DEFAULT_ANALYTICS_ENABLED);
export const analyticsEnabledAtom = atom(get => get(baseAnalyticsEnabledAtom), async (get, set, nextEnabled) => {
...
baseAnalyticsEnabledAtom.onMount = (setAtom) => {
    void storageAdapter.get(ANALYTICS_ENABLED_STORAGE_KEY, DEFAULT_ANALYTICS_ENABLED, analyticsEnabledSchema).then(setAtom);
    const unwatch = storageAdapter.watch(ANALYTICS_ENABLED_STORAGE_KEY, setAtom);
