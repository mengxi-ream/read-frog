 ...
 */
let writeVersion = 0;
export const writeConfigAtom = atom(null, async (get, set, patch) => {
    // ─────────────────────────────────────────────────────────────────────────
    ...
    const task = writeQueue.then(async () => {
        ...
        const configInStorage = await storageAdapter.get(CONFIG_STORAGE_KEY, DEFAULT_CONFIG, configSchema);
...
configAtom.onMount = (setAtom) => {
    ...
    void storageAdapter.get(CONFIG_STORAGE_KEY, DEFAULT_CONFIG, configSchema).then((value) => {
        if (!didReceiveStorageUpdate) {
            setAtom(value);
        }
    });
    // Watch for changes from other extension contexts (popup, options page, other tabs)
    const unwatch = storageAdapter.watch(CONFIG_STORAGE_KEY, (value) => {
    ...
    const handleVisibilityChange = () => {
        if (document.visibilityState === "visible") {
            ...
            void storageAdapter.get(CONFIG_STORAGE_KEY, DEFAULT_CONFIG, configSchema).then(setAtom);
        }
    };
...
export function getConfigFieldAtom(key) {
    ...
    return atom(get => get(sliceAtom), (_get, set, newVal) => set(writeConfigAtom, { [key]: newVal }));
}
function buildConfigFieldsAtomMap(cfg) {
    const res = {};
    const add = (key) => {
