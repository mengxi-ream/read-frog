...
const OPENAI_COMPATIBLE_PROVIDER_TYPES = new Set(CUSTOM_LLM_PROVIDER_TYPES);
const OPENAI_COMPATIBLE_OPTION_ALIASES = {
    reasoning_effort: "reasoningEffort",
    verbosity: "textVerbosity",
};
function normalizeUserProviderOptions(provider, userOptions) {
    if (!OPENAI_COMPATIBLE_PROVIDER_TYPES.has(provider)) {
        return userOptions;
    }
    let changed = false;
    const normalizedOptions = { ...userOptions };
 ...
 */
export function getRecommendedProviderOptionsMatch(model) {
 ...
 */
export function getProviderOptionsWithOverride(model, provider, userOptions) {
