...
export function parseSubtitleRenditions(
  manifestUrl: string,
  manifestText: string,
  ...
  return manifestText
    ...
    .flatMap((attributes) => {
      ...
      if (attributes.TYPE !== "SUBTITLES" || !uri) {
        return []
      }

      const resolvedUri = resolveUrl(uri, manifestUrl)
      if (!resolvedUri) {
        return []
      }
...
function matchesTextTrack(
  rendition: XcomSubtitleRendition,
  selectedTrack: XcomTextTrackSnapshot,
): boolean {
  ...
  return (
    (!!selectedLanguage && selectedLanguage === renditionLanguage) ||
...
export function orderSubtitleRenditions(
  ...
  const preferred = [selected, defaultTrack, autoselectTrack].filter(
    (track): track is XcomSubtitleRendition => !!track,
  )

  return [...preferred, ...renditions].filter(
    (track, index, tracks) => tracks.indexOf(track) === index,
  )
}
