...
describe("subtitles translator", () => {
    ...
    it("passes title and description when AI content awareness is disabled", async () => {
        ...
        expect(getSubtitlesTranslatePromptMock).toHaveBeenCalledWith(expect.any(String), expect.any(String), expect.objectContaining({
            context: {
    ...
    it("uses an explicit config snapshot without reading current settings again", async () => {
        ...
        expect(sendMessageMock).toHaveBeenCalledWith("enqueueSubtitlesTranslateRequest", expect.objectContaining({ langConfig: configSnapshot.language }));
    });
