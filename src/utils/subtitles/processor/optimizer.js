...
function isQualityPoor(fragments) {
...
function processSubtitles(fragments, language, usePause = false) {
    const result = [];
    const buffer = [];
    let bufferLength = 0;
...
function shouldKeepBoundary(left, right) {
...
function rebalanceToTargetRange(fragments, language) {
...
export function optimizeSubtitles(fragments, language) {
    if (fragments.length === 0)
        return [];
