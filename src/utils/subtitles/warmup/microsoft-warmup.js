...
export async function microsoftWarmupTranslate(fragments, fromLang, toLang) {
    if (fragments.length === 0) {
        return [];
    }
    const chunks = chunkFragments(fragments);
    const results = await Promise.allSettled(chunks.map(chunk => sendMessage("microsoftBatchTranslate", {
