...
function makeFragment(text, start, end) {
...
describe("microsoftWarmupTranslate", () => {
    ...
    it("translates a single fragment", async () => {
        mockSendMessage.mockResolvedValueOnce(["你好"]);
    ...
    it("translates multiple fragments in one chunk", async () => {
        mockSendMessage.mockResolvedValueOnce(["你好", "世界", "今天"]);
        const fragments = [
    ...
    it("does not mutate input fragments", async () => {
        mockSendMessage.mockResolvedValueOnce(["你好"]);
        const original = { text: "Hello", start: 0, end: 1000 };
