...
export const SITE_CONTROL_URL_WINDOW_KEY = "__READ_FROG_SITE_CONTROL_URL__";
export function isSiteEnabled(url, config) {
    if (!config)
        return true;
...
export function resolveEffectiveSiteControlUrl(url, injectedSiteControlUrl) {
    return injectedSiteControlUrl ?? url;
}
export function getEffectiveSiteControlUrl(url) {
    if (typeof window === "undefined") {
        return url;
    }
    return resolveEffectiveSiteControlUrl(url, window.__READ_FROG_SITE_CONTROL_URL__);
}
export function clearEffectiveSiteControlUrl() {
    if (typeof window === "undefined") {
        return;
    }
