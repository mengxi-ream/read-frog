 ...
 */
export async function syncMergedConfig(mergedConfig, email) {
    try {
        const now = Date.now();
        ...
        await setLastSyncConfigAndMeta(validatedConfig, { schemaVersion: CONFIG_SCHEMA_VERSION, lastModifiedAt: now, email });
    ...
    try {
        ...
        if (email !== lastSyncedConfigValueAndMeta?.meta.email) {
            if (remoteConfigValueAndMeta) {
                ...
                await setLastSyncConfigAndMeta(remoteConfigValueAndMeta.value, { ...remoteConfigValueAndMeta.meta, email, lastSyncedAt: now });
                ...
                await setLastSyncConfigAndMeta(localConfigValueAndMeta.value, { ...localConfigValueAndMeta.meta, email, lastSyncedAt: now });
        ...
        if (localChangedSinceSync && remoteChangedSinceSync) {
            ...
            if (sameLocalAndRemote) {
                ...
                await setLastSyncConfigAndMeta(mergedConfigValueAndMeta.value, { ...mergedConfigValueAndMeta.meta, email, lastSyncedAt: now });
