...
function createConfigValueAndMeta(config, meta = {}) {
    return {
        value: config,
        meta: {
            schemaVersion: 1,
...
function createLastSyncedConfigValueAndMeta(config, meta = {}) {
    return {
        value: config,
        meta: {
            schemaVersion: 1,
...
describe("syncConfig", () => {
    ...
    describe("1. First Login (No Previous Sync)", () => {
        it("1.1 should download when remote exists", async () => {
            ...
            vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(localConfig, { lastModifiedAt: 1000 }));
            ...
            expect(setLastSyncConfigAndMeta).toHaveBeenCalledWith(remoteConfig, expect.objectContaining({ email: "a@test.com" }));
        ...
        it("1.2 should upload when no remote exists", async () => {
            ...
            expect(setRemoteConfigAndMeta).toHaveBeenCalledWith(createConfigValueAndMeta(localConfig, { lastModifiedAt: 1000 }));
            expect(setLastSyncConfigAndMeta).toHaveBeenCalledWith(localConfig, expect.objectContaining({ email: "a@test.com" }));
    ...
    describe("2. Account Switch (User A � User B)", () => {
        it("2.1 should download when remote B exists", async () => {
            ...
            vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(localConfig, { lastModifiedAt: 2000 }));
            vi.mocked(getLastSyncedConfigAndMeta).mockResolvedValue(createLastSyncedConfigValueAndMeta(localConfig, { email: "a@test.com", lastModifiedAt: 1000 }));
            vi.mocked(getRemoteConfigAndMetaWithUserEmail).mockResolvedValue({
            ...
            expect(setLastSyncConfigAndMeta).toHaveBeenCalledWith(remoteBConfig, expect.objectContaining({ email: "b@test.com" }));
            expect(setRemoteConfigAndMeta).not.toHaveBeenCalled();
        });
        it("2.2 should upload when no remote B exists", async () => {
            ...
            expect(setLastSyncConfigAndMeta).toHaveBeenCalledWith(localConfig, expect.objectContaining({ email: "b@test.com" }));
            expect(setLocalConfigAndMeta).not.toHaveBeenCalled();
        });
    });
    describe("3. Same Account Sync (User A continues)", () => {
        describe("3.1 Both Changed", () => {
            it("3.1.1 should return same-changes when content is identical", async () => {
                ...
                vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(changedConfig, { lastModifiedAt: 2000 }));
                vi.mocked(getLastSyncedConfigAndMeta).mockResolvedValue(createLastSyncedConfigValueAndMeta(baseConfig, { email: "a@test.com", lastModifiedAt: 1000 }));
                vi.mocked(getRemoteConfigAndMetaWithUserEmail).mockResolvedValue({
                ...
                expect(setRemoteConfigAndMeta).toHaveBeenCalled();
                expect(setLastSyncConfigAndMeta).toHaveBeenCalledWith(changedConfig, expect.objectContaining({ email: "a@test.com" }));
            });
        ...
        it("3.2 should download when only remote changed", async () => {
            const baseConfig = createTestConfig({ setting1: "base" });
            const remoteConfig = createTestConfig({ setting1: "remote-changed" });
            vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(baseConfig, { lastModifiedAt: 1000 }));
            vi.mocked(getLastSyncedConfigAndMeta).mockResolvedValue(createLastSyncedConfigValueAndMeta(baseConfig, { email: "a@test.com", lastModifiedAt: 1000 }));
            vi.mocked(getRemoteConfigAndMetaWithUserEmail).mockResolvedValue({
                configValueAndMeta: createConfigValueAndMeta(remoteConfig, { lastModifiedAt: 2000 }),
                email: "a@test.com",
        ...
        it("3.4 should return no-change when nothing changed", async () => {
            ...
            vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(config, { lastModifiedAt: 1000 }));
            vi.mocked(getLastSyncedConfigAndMeta).mockResolvedValue(createLastSyncedConfigValueAndMeta(config, { email: "a@test.com", lastModifiedAt: 1000 }));
            vi.mocked(getRemoteConfigAndMetaWithUserEmail).mockResolvedValue({
    ...
    describe("4. Error Handling", () => {
        ...
        it("should return error status when getRemoteConfigAndMetaWithUserEmail throws", async () => {
            ...
            vi.mocked(getLocalConfigAndMeta).mockResolvedValue(createConfigValueAndMeta(createTestConfig()));
            vi.mocked(getLastSyncedConfigAndMeta).mockResolvedValue(null);
        ...
        it("should propagate ConfigVersionTooNewError from remote config", async () => {
            ...
            const result = await syncConfig();
            expect(result.status).toBe("error");
            expect(result.error).toBeInstanceOf(ConfigVersionTooNewError);
            expect(result.error.message).toBe("Please upgrade");
        });
    });
});
