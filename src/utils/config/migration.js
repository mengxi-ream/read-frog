 ...
 */
const modules = import.meta.glob(["./migration-scripts/v*-to-v*.ts"], { eager: true, import: "migrate" });
export const migrationScripts = Object.fromEntries(Object.entries(modules).map(([path, migrate]) => {
...
export async function runMigration(version, config) {
...
export async function migrateConfig(originalConfig, originalConfigSchemaVersion) {
