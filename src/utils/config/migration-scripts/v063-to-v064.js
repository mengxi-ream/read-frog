 ...
 */
function replaceTranslatePromptTokens(text) {
...
function replaceCustomActionPromptTokens(text) {
    if (typeof text !== "string") {
        return text;
    }
    return text
...
function isDictionaryLikeAction(action) {
    return Array.isArray(action?.outputSchema)
        && action.outputSchema.some((field) => field?.id === "dictionary-context"
...
function migrateDictionaryText(text) {
    if (typeof text !== "string") {
        return text;
    }
    return text
...
function migrateDictionaryField(field) {
...
function migrateCustomActions(actions) {
