...
function isTranslateProviderName(providerName) {
    return typeof providerName === "string" && TRANSLATE_PROVIDER_TYPES.includes(providerName);
}
function isLLMProviderName(providerName) {
    return typeof providerName === "string" && LLM_PROVIDER_TYPES.includes(providerName);
}
function migrateProviderModel(provider) {
...
function migrateProviderModels(providersConfig) {
    if (!Array.isArray(providersConfig)) {
        return [];
    }
    return providersConfig.map(migrateProviderModel);
}
function getTranslateProviderIds(providersConfig) {
...
function getLLMProviderIds(providersConfig) {
    return providersConfig
        .filter(provider => isObject(provider)
        && typeof provider.id === "string"
...
function resolveTranslateProviderId(oldConfig, translateProviderIds) {
...
function resolveVocabularyInsightProviderId(oldConfig, fallbackProviderId, llmProviderIds) {
...
function normalizeFeatureConfig(rawFeatureConfig, fallbackProviderId, allowedProviderIds) {
...
function migrateSelectionToolbarFeatures(oldConfig, providersConfig) {
    ...
    const features = {
        translate: normalizeFeatureConfig(oldFeatures.translate, defaultTranslateProviderId, translateProviderIds),
        vocabularyInsight: normalizeFeatureConfig(oldFeatures.vocabularyInsight, defaultVocabularyInsightProviderId, llmProviderIds),
    };
    return {
...
function reorderProviders(providersConfig) {
...
function removeReadConfig(oldConfig) {
...
function migrateInputTranslation(oldConfig, translateProviderIds) {
...
function migrateVideoSubtitles(oldConfig, translateProviderIds) {
