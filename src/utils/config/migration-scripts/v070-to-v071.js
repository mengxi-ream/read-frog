...
function findFirstIndex(value, needles, fromIndex) {
    let firstIndex = -1;
...
function isValidLiteralHTTPURL(value) {
    try {
...
function getLiteralHostname(value) {
    try {
...
function buildURL(parsed, pathname, search = parsed.search) {
    return `${parsed.origin}${pathname}${search}${parsed.hash}`;
}
function trimPathTrailingSlashes(pathname) {
    return pathname.replace(/\/+$/, "");
}
function hasTranslatePath(pathname) {
    return trimPathTrailingSlashes(pathname).endsWith("/translate");
}
function appendTranslatePath(pathname) {
    const cleanPathname = trimPathTrailingSlashes(pathname);
    return `${cleanPathname || ""}/translate`;
}
function appendAPIKeyPlaceholderAndTranslate(pathname) {
    const cleanPathname = trimPathTrailingSlashes(pathname);
...
function isTokenParam(param) {
    return param.name === "token";
}
function getSingleTokenQueryValue(search) {
...
function replaceSingleTokenQueryValue(search) {
...
function migratePathTokenProvider(provider, parsed, apiKey, hostname) {
...
function migratePlaceholderURL(provider, parsed) {
...
function migrateQueryTokenURL(provider, parsed, apiKey, token) {
    ...
    return {
        ...provider,
        apiKey: token,
        baseURL: buildURL(parsed, hasTranslatePath(parsed.pathname) ? parsed.pathname : appendTranslatePath(parsed.pathname), replaceSingleTokenQueryValue(parsed.search)),
    };
}
function migrateProvider(provider) {
    ...
    return {
        ...provider,
        baseURL: buildURL(parsed, apiKey
