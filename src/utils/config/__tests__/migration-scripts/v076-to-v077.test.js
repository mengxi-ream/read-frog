...
describe("v076-to-v077 migration", () => {
    it("renames subtitle title prompt tokens back to webpage title", () => {
        ...
        expect(migrated.videoSubtitles.customPromptsConfig.patterns[0].systemPrompt).toBe("Use {{webTitle}} and {{videoSummary}}");
        expect(migrated.videoSubtitles.customPromptsConfig.patterns[0].prompt).toBe("Title: {{webTitle}}\nSummary: {{videoSummary}}\n{{input}}");
        expect(migrated.translate.customPromptsConfig.patterns[0].systemPrompt).toBe("Keep {{videoTitle}} outside subtitle prompts");
    });
