...
import { computeLanguageDetectionFallbackAfterDeletion, computeProviderFallbacksAfterDeletion, computeSelectionToolbarCustomActionFallbacksAfterDeletion, findFeatureMissingProvider, resolveLanguageDetectionConfigForModeChange, } from "../helpers";
...
describe("feature providers", () => {
    ...
    describe("computeSelectionToolbarCustomActionFallbacksAfterDeletion", () => {
        it("reassigns affected custom actions to the first enabled llm provider", () => {
            ...
            const result = computeSelectionToolbarCustomActionFallbacksAfterDeletion("deleted-provider", config, remainingProviders);
    ...
    describe("resolveLanguageDetectionConfigForModeChange", () => {
        it("assigns the first enabled llm provider when switching from basic to llm", () => {
            const result = resolveLanguageDetectionConfigForModeChange(DEFAULT_CONFIG.languageDetection, "llm", DEFAULT_CONFIG.providersConfig);
            expect(result).toEqual({
                mode: "llm",
                providerId: "openai-default",
            });
        ...
        it("keeps the current provider when it is already an enabled llm provider", () => {
            const result = resolveLanguageDetectionConfigForModeChange({
                mode: "basic",
                providerId: "google-default",
            }, "llm", DEFAULT_CONFIG.providersConfig);
            expect(result).toEqual({
                mode: "llm",
                providerId: "google-default",
            });
        ...
        it("returns null when there is no enabled llm provider", () => {
            const result = resolveLanguageDetectionConfigForModeChange(DEFAULT_CONFIG.languageDetection, "llm", [
                {
                    ...getProviderById("openai-default"),
                    enabled: false,
                },
