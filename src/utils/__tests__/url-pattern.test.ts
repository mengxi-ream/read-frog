 ...
 * `stored` is what the migration (and `normalizeUserSitePattern`) produces;
...
describe("website-list patterns keep their old meaning", () => {
  it.each(GOLDEN)("$pattern", ({ pattern, stored, matches }) => {
    expect(normalizeUserSitePattern(pattern)).toBe(stored)
...
describe("normalizeUserSitePattern", () => {
  it("expands a bare host to cover its subdomains", () => {
    expect(normalizeUserSitePattern("example.com")).toBe("*.example.com")
    expect(normalizeUserSitePattern("  Example.COM  ")).toBe("*.example.com")
  })

  it("leaves anything that is already a pattern alone", () => {
    expect(normalizeUserSitePattern("*.example.com")).toBe("*.example.com")
    expect(normalizeUserSitePattern("example.com/docs/*")).toBe("example.com/docs/*")
    expect(normalizeUserSitePattern("https://example.com")).toBe("https://example.com")
    expect(normalizeUserSitePattern("*")).toBe("*")
  })

  it("leaves a colon alone rather than producing *.localhost:5173", () => {
    expect(normalizeUserSitePattern("localhost:5173")).toBe("localhost:5173")
    expect(normalizeUserSitePattern("[::1]")).toBe("[::1]")
  })
  ...
  it("is idempotent, so re-running the migration changes nothing", () => {
    for (const input of ["example.com", "*.example.com", "example.com/docs", "[::1]", ""]) {
      const once = normalizeUserSitePattern(input)
      expect(normalizeUserSitePattern(once)).toBe(once)
    }
  })
})
