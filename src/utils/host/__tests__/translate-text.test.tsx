...
describe('translate-text', () => {
  ...
  describe('executeTranslate', () => {
    ...
    it('should return empty string for empty/whitespace input', async () => {
      expect(await executeTranslate('', langConfig, providerConfig, { promptResolver: getTranslatePrompt })).toBe('')
      expect(await executeTranslate(' ', langConfig, providerConfig, { promptResolver: getTranslatePrompt })).toBe('')
      expect(await executeTranslate('\n', langConfig, providerConfig, { promptResolver: getTranslatePrompt })).toBe('')
      expect(await executeTranslate(' \n ', langConfig, providerConfig, { promptResolver: getTranslatePrompt })).toBe('')
      expect(await executeTranslate(' \n \t', langConfig, providerConfig, { promptResolver: getTranslatePrompt })).toBe('')
    })
    ...
    it('should handle zero-width spaces correctly', async () => {
      ...
      const result = await executeTranslate('\u200B hello \u200B', langConfig, providerConfig, { promptResolver: getTranslatePrompt })
    ...
    it('should trim translation result', async () => {
      ...
      const result = await executeTranslate('test input', langConfig, providerConfig, { promptResolver: getTranslatePrompt })
