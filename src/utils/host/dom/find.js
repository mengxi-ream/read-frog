 ...
 */
function findElementAt(root, point) {
    const { x, y } = point;
...
export function findNearestAncestorBlockNodeFor(element) {
 ...
 */
export function findNearestAncestorBlockNodeAt(point) {
    const currentNode = findElementAt(document, point);
    if (!currentNode)
        return null;
    return findNearestAncestorBlockNodeFor(currentNode);
}
export function deepQueryTopLevelSelector(element, selectorFn) {
...
export async function unwrapDeepestOnlyHTMLChild(element) {
    const config = await getLocalConfig() ?? DEFAULT_CONFIG;
    let currentElement = element;
    while (currentElement) {
        smashTruncationStyle(currentElement);
        const shouldKeepNode = (child) => {
 ...
 */
export function findTranslatedContentWrapper(node) {
