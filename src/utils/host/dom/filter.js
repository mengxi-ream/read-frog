import { BLOCK_ATTRIBUTE, BLOCK_CONTENT_CLASS, CONTENT_WRAPPER_CLASS, INLINE_ATTRIBUTE, INLINE_CONTENT_CLASS, NOTRANSLATE_CLASS, } from "@/utils/constants/dom-labels";
...
export function isShallowInlineTransNode(node) {
...
export function isShallowInlineHTMLElement(element) {
...
export function isShallowBlockTransNode(node) {
    if (isTextNode(node)) {
        return false;
    }
    else if (isHTMLElement(node)) {
...
export function isShallowBlockHTMLElement(element) {
    const computedStyle = window.getComputedStyle(element);
    if (FORCE_BLOCK_TAGS.has(element.tagName)) {
        return true;
    }
...
export function isCustomDontWalkIntoElement(element) {
...
export function isCustomForceBlockTranslation(element) {
...
export function isDontWalkIntoButTranslateAsChildElement(element) {
...
export function isDontWalkIntoAndDontTranslateAsChildElement(element, config) {
    ...
    const dontWalkCSS = window.getComputedStyle(element).display === "none"
        || window.getComputedStyle(element).visibility === "hidden";
    const dontWalkHidden = element.hidden;
    const dontWalkAriaHidden = element.getAttribute("aria-hidden") === "true";
    const dontWalkVisuallyHidden = ["sr-only", "visually-hidden"].some(cls => element.classList.contains(cls));
...
export function isInlineTransNode(node) {
    if (isTextNode(node)) {
        return true;
    }
    return node.hasAttribute(INLINE_ATTRIBUTE);
}
export function isBlockTransNode(node) {
    if (isTextNode(node)) {
        return false;
    }
...
export function isTransNode(node) {
    return isHTMLElement(node) || isTextNode(node);
}
export function isIFrameElement(node) {
    return node.nodeType === Node.ELEMENT_NODE
        && node.nodeName === "IFRAME";
}
export function isTranslatedWrapperNode(node) {
 ...
 */
export function isTranslatedContentNode(node) {
 ...
 */
export function hasNoWalkAncestor(element, config) {
    let current = element.parentElement;
    while (current) {
