...
import { isCustomForceBlockTranslation, isDontWalkIntoAndDontTranslateAsChildElement, isDontWalkIntoButTranslateAsChildElement, isHTMLElement, isShallowBlockHTMLElement, isShallowInlineHTMLElement, isTextNode, } from "./filter";
const NON_NEWLINE_WHITESPACE_RE = /[^\S\n]/;
export function extractTextContent(node, config) {
    if (isTextNode(node)) {
    ...
    return childNodes.reduce((text, child) => {
...
export function walkAndLabelElement(element, walkId, config) {
