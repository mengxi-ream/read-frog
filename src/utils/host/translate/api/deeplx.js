...
export async function deeplxTranslate(sourceText, fromLang, toLang, providerConfig, options) {
...
async function fetchViaBackground(url, body) {
...
async function fetchDirect(url, body) {
...
async function parseDeepLXResponse(resp) {
    if (!resp.ok) {
        ...
        throw new Error(`DeepLX translation request failed: ${resp.status} ${resp.statusText}${errorText ? ` - ${errorText}` : ""}`);
    }
    try {
        ...
        throw new Error(`Failed to parse DeepLX translation response: ${error.message}`);
    }
}
export function buildDeepLXUrl(baseURL, apiKey) {
