import { CONTENT_WRAPPER_CLASS, NOTRANSLATE_CLASS, TRANSLATION_MODE_ATTRIBUTE, WALKED_ATTRIBUTE, } from "../../../constants/dom-labels";
...
export async function translateNodes(nodes, walkId, toggle = false, config, forceBlockTranslation = false) {
...
export async function translateNodesBilingualMode(nodes, walkId, config, toggle = false, forceBlockTranslation = false) {
    ...
    try {
        ...
        const targetNode = transNodes.length === 1 && isBlockTransNode(lastNode) && isHTMLElement(lastNode)
        ...
        await insertTranslatedNodeIntoWrapper(translatedWrapperNode, targetNode, translatedText, config.translate.translationNodeStyle, forceBlockTranslation);
    }
    finally {
        transNodes.forEach(node => translatingNodes.delete(node));
    }
}
export async function translateNodeTranslationOnlyMode(nodes, walkId, config, toggle = false) {
    const isTransNodeAndNotTranslatedWrapper = (node) => {
    ...
    try {
        ...
        const getStringFormatFromNode = (node) => {
            if (isTextNode(node)) {
