...
async function isTextAlreadyInTargetLanguage(text, targetCode) {
...
async function getWebPagePromptContext(providerConfig, enableAIContentAware, includeSummary) {
...
async function translateTextUsingPageConfig(config, text, options = {}) {
    const preparedText = prepareTranslationText(text);
    if (preparedText === "") {
        return "";
    }
    const providerConfig = resolveProviderConfig(config, "translate");
    if (config.translate.page.enableTargetLanguageSkip
    ...
    if (skipLanguages.length > 0 && preparedText.length >= MIN_LENGTH_FOR_SKIP_LLM_DETECTION) {
        const shouldSkip = await shouldSkipByLanguage(preparedText, skipLanguages, config.languageDetection.mode === "llm");
        if (shouldSkip) {
 ...
 */
export async function translateTextForPage(text) {
 ...
 */
export async function translateTextForPageTitle(text) {
    const config = await getConfigOrThrow();
    const providerConfig = resolveProviderConfig(config, "translate");
...
async function resolveInputLang(lang, globalLangConfig) {
    if (lang === "sourceCode") {
 ...
 */
export async function translateTextForInput(text, fromLang, toLang) {
    const config = await getConfigOrThrow();
