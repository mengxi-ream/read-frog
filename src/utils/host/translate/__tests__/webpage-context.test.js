...
describe("getOrCreateWebPageContext", () => {
    ...
    it("converts Defuddle HTML content when a separate markdown field is missing", async () => {
        ...
        expect(mockCreateMarkdownContent).toHaveBeenCalledWith("<h1>Readable page body</h1>", window.location.href);
    });
    ...
    it("falls back to body text when Defuddle parsing fails", async () => {
        ...
        expect(mockWarn).toHaveBeenCalledWith("Defuddle parsing failed, falling back to body text:", expect.any(Error));
    });
});
