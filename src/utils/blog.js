...
function getBilibiliVideoIdFromParsedUrl(parsedUrl) {
 ...
 */
export async function saveLastViewedBlogDate(date) {
 ...
 */
export async function getLastViewedBlogDate() {
    const dateStr = await storage.getItem(`local:${LAST_VIEWED_BLOG_DATE_KEY}`);
 ...
 */
export function hasNewBlogPost(latestViewedDate, latestDate) {
    if (!latestDate)
        return false;
    if (!latestViewedDate)
        return true;
...
export function buildBilibiliEmbedUrl(url) {
...
export function resolveBlogLocale(uiLocale) {
...
export function getBlogLocaleFromUILanguage() {
 ...
 * ```
 */
export async function getLatestBlogDate(apiUrl = "https://readfrog.app/api/blog/latest", locale = "en", extensionVersion, useCache = true) {
    try {
