...
import { EDGE_TTS_ENDPOINT_URL, EDGE_TTS_SIGNATURE_APP_ID, getEdgeTTSSignatureSecretBase64, } from "./constants";
...
async function hmacSha256(key, data) {
    ...
    const cryptoKey = await crypto.subtle.importKey("raw", keyBuffer, { name: "HMAC", hash: { name: "SHA-256" } }, false, ["sign"]);
    const signatureBuffer = await crypto.subtle.sign("HMAC", cryptoKey, new TextEncoder().encode(data));
    return new Uint8Array(signatureBuffer);
}
export function buildSignatureDate(date = new Date()) {
    return `${date.toUTCString().replace(GMT_SUFFIX_PATTERN, "").trim().toLowerCase()} GMT`;
}
export async function generateTranslatorSignature(url = EDGE_TTS_ENDPOINT_URL, now = new Date()) {
    try {
