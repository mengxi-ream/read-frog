import { EDGE_TTS_MAX_RETRIES, EDGE_TTS_OUTPUT_FORMAT, EDGE_TTS_RETRY_BASE_DELAY_MS, EDGE_TTS_USER_AGENT, } from "./constants";
...
function isConcatenationSafeOutputFormat(outputFormat) {
    const normalized = outputFormat.toLowerCase();
    return normalized.includes("mp3") || normalized.startsWith("raw-");
}
function assertChunkConcatSupported(chunkRequests) {
    ...
    for (let index = 0; index < requestedFormats.length; index++) {
        ...
        if (!isConcatenationSafeOutputFormat(requestedFormat)) {
            throw new EdgeTTSError("SYNTH_REQUEST_FAILED", `Output format "${requestedFormat}" is not safe for multi-chunk concatenation`);
        }
        if (requestedFormatNormalized !== firstFormatNormalized) {
            throw new EdgeTTSError("SYNTH_REQUEST_FAILED", `Mixed output formats are not supported for multi-chunk concatenation: "${firstFormat}" and "${requestedFormat}"`);
        }
    }
}
async function synthesizeChunk(request) {
...
export async function synthesizeEdgeTTSChunkWithRetry(request) {
    let lastError;
...
export async function combineEdgeTTSAudioChunks(chunkRequests) {
