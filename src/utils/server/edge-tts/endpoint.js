...
import { EDGE_TTS_CLIENT_VERSION, EDGE_TTS_DEFAULT_TOKEN_TTL_MS, EDGE_TTS_ENDPOINT_URL, EDGE_TTS_HOME_REGION, EDGE_TTS_TOKEN_REFRESH_BEFORE_EXPIRY_MS, EDGE_TTS_USER_AGENT, EDGE_TTS_USER_ID, } from "./constants";
...
function hasValidCachedToken(now = Date.now()) {
    if (!tokenInfo) {
        return false;
    }
    return now < (tokenInfo.expiredAt - EDGE_TTS_TOKEN_REFRESH_BEFORE_EXPIRY_MS);
}
function hasUnexpiredCachedToken(now = Date.now()) {
    if (!tokenInfo) {
        return false;
    }
    return now < tokenInfo.expiredAt;
}
function ensureEndpointPayload(data) {
...
export async function getEdgeTTSEndpointToken() {
...
export function clearEdgeTTSTokenCache() {
    tokenInfo = null;
}
