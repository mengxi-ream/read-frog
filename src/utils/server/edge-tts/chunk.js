...
function alignToCodePointBoundary(text, index) {
...
function findLargestSliceByBytes(text, maxBytes) {
    let low = 1;
...
function adjustBySoftBoundary(text, candidate) {
...
function adjustByEntityBoundary(text, candidate) {
...
export function splitTextByUtf8Bytes(text, maxChunkBytes = EDGE_TTS_MAX_CHUNK_BYTES, maxChunks = EDGE_TTS_MAX_CHUNKS) {
    ...
    while (remaining.length > 0) {
        ...
        if (chunks.length > maxChunks) {
            throw new EdgeTTSError("TEXT_TOO_LONG", `Text is too long for Edge TTS (max ${maxChunks} chunks at ${maxChunkBytes} bytes each)`);
        }
