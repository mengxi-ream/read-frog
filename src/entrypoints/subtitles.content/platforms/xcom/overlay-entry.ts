...
function clearStaleEntryPoints(currentContainer: HTMLElement): void {
  document.querySelectorAll<HTMLElement>(XCOM_PLAYER_CONTAINER_SELECTOR).forEach((container) => {
...
export function clearXcomOverlayEntryPoints(): void {
  document
    .querySelectorAll<HTMLElement>(XCOM_CONTROLS_CONTAINER_SELECTOR)
    .forEach((container) => container.remove())

  document
    .querySelectorAll<HTMLElement>(XCOM_PLAYER_CONTAINER_SELECTOR)
...
export function ensureXcomOverlayEntryPoint(): boolean {
  ...
  const existingEntryPoint = videoContainer.querySelector<HTMLElement>(
    XCOM_CONTROLS_CONTAINER_SELECTOR,
  )
