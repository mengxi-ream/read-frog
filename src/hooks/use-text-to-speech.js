...
function toSignedValue(value, unit) {
    return `${value >= 0 ? "+" : ""}${value}${unit}`;
}
export function selectTTSVoice(ttsConfig, detectedLanguage, forcedVoice) {
    if (forcedVoice) {
        return forcedVoice;
    }
    if (detectedLanguage && detectedLanguage in ttsConfig.languageVoices) {
        return ttsConfig.languageVoices[detectedLanguage] ?? ttsConfig.defaultVoice;
    }
    return ttsConfig.defaultVoice;
}
async function resolveVoiceForText(text, ttsConfig, enableLLM, forcedVoice) {
    if (forcedVoice) {
...
function getTTSFriendlyErrorDescription(error) {
...
async function synthesizeEdgeTTSAudioChunk(chunk, voice, ttsConfig) {
...
export function useTextToSpeech(surface = ANALYTICS_SURFACE.SELECTION_TOOLBAR) {
    ...
    const playMutation = useMutation({
        ...
        mutationFn: async ({ text, ttsConfig, analyticsContext, forcedVoice }) => {
            ...
            const playChunk = async (audioChunk) => {
                setIsPlaying(true);
                try {
    ...
    const play = (text, ttsConfig, options) => {
